...
function ReserveDetailSheet({ reserve, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 300, background: BG, overflowY: "auto", fontFamily: "inherit", animation: "fadeIn 0.2s ease" }}>
      {/* Nav : retour + partager (mockup) */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "calc(8px + env(safe-area-inset-top, 0px)) 16px 14px" }}>
        ...
        <button
          onClick={() => { const t = `${reserve.code || "Réserve"} — ${reserve.description || ""}`; if (navigator.share) { navigator.share({ title: reserve.code || "Réserve", text: t }).catch(() => {}); } else { try { navigator.clipboard.writeText(t); } catch { /* ignore */ } } }}
          aria-label="Partager / exporter"
          style={{ width: 40, height: 40, minWidth: 40, minHeight: 40, flexShrink: 0, borderRadius: "50%", background: WH, border: `1px solid #EFEDEB`, display: "flex", alignItems: "center", justifyContent: "center", color: TX2, cursor: "pointer" }}
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={TX2} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" /><polyline points="16 6 12 2 8 6" /><line x1="12" y1="2" x2="12" y2="15" /></svg>
        </button>
      </div>
      {/* Titre */}
